...
path = '/home/martinha/PycharmProjects/phd/breast_cancer_imc/breast_data/'
path_images = os.path.join(path, 'imc_metabric2022/MBTMEIMCPublic/Images/')
output_path = os.path.join(path, 'imc_metabric/noise2void')
if not os.path.exists(output_path):
    os.mkdir(output_path)

# img_arr = IP.parse_image(img_path) # (679, 665, 39)
# clean_arr = np.empty(img_arr.shape)
names_images = [f for f in os.listdir(path_images) if 'FullStack' in f]
files = [str(path_images + sub) for sub in names_images][:30]
...
for channel in range(39):
    ...
    del X_val
    print(len(images_ch))
    pred_train = model.predict(np.array(images_ch), axes='YX')
...
for clean_img in clean_arr:
    ...
    denoise_img2_t = np.moveaxis(clean_img, -1, 0)
    tifffile.imwrite(output_path + 'noise2void_image_{}.tiff'.format(i),
                     denoise_img2_t,
