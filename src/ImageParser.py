...
def parse_image(img_path: str) -> np.array:
    """Load an image and its annotation.
     Squeezes and moves axis. Returns numpy array (X,Y, channels)
...
def parse_image_pages(img_path: str) -> np.array:
    """
    Loads tiff files with pages and not fullstack
    :param img_path: str    Image location.
...
def parse_image_pages_namesCH(img_path):
    """
    Loads tiff files with pages and not fullstack.
    Retrieves page names.
    :param img_path: str    Image location.
    :return: numpy array of image and list of channel names
    """
    im = []
    ch_names = []
    with tifffile.TiffFile(img_path) as tif:
